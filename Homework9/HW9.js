...
function AgeFilter(array, age){
    let res=[];
    for(obj of array){
        if(obj.age>age) res.push(obj)
    }
    return res
}

function GandC(array,gender,country){
    let res=0;
    for(obj of array){
        if(obj.gender===gender&&obj.country===country) res++;
    }
    return res;
}
...
function JobCount(array){
    var res={};
    for(obj of array){
        let job=array.job
        if(array.job!=undefined) res[job]++
        else res.job=1
    }
    return res
}

function isMarried(array){
    let res={
        'Đã lập gia đình': 0
    };
    for(obj of array){
        if(obj.is_married==true) res['Đã lập gia đình']++;
    }
    res['Chưa kết hôn']=array.length-res['Đã lập gia đình'];
    return res;
}
console.log(isMarried(users));
function info1(array) {
    var res=[];
    for(obj of array){
        let obj2 = {
            id : obj.id,
...
function info2(array,info){
    var res=[];
    for(obj of array){
        let obj2 = {};
        for(let i=0;i<info.length;i++){
            obj2[info[i]]=obj.field
        }
        res.push(obj2);
    }
    return res;
...
// function mappingJob(array, jobName) {
//     let result = array.filter(x => x.job == jobName).length;
//     return result;
// }

// function displayCountJob(array, arrayNameJob) {
//     let obj = {};
//     for (let i=0; i < arrayNameJob.length; i++) {
//         let nameJob = arrayNameJob[i];        
//         obj[nameJob] = mappingJob(array, nameJob);
//     }
//     console.log(obj);
// }

displayCountJob(users,users.map(item => item.job)
.filter((value, index, self) => self.indexOf(value) === index));

function FindUserByKeyWord(array,keyword){
    var res=[]
    var KW=keyword.toLowerCase();
    for(obj of array){
        if(obj.first_name.includes(KW)||obj.last_name.includes(KW))
        {
            res.push(obj)
        }
    }
...
// console.log(FindUserByKeyWord(users,"Am"))
